...
drop table if exists geodata.networklines_0_0;
drop table if exists geodata.networklines_1_0;
drop table if exists geodata.networklines_2_0;
drop table if exists geodata.networklines_3_0;
...
SELECT id, ST_Force3D(ST_Transform(ST_Force2D(st_geometryN(geom, 1)),3857)) AS geom,
  ...
  INTO geodata.networklines_0_0
  FROM django.routing_networklines_0_0;

SELECT id, ST_Force3D(ST_Transform(ST_Force2D(st_geometryN(geom, 1)),3857)) AS geom,
  network_type, cost::FLOAT, 0.0 AS reverse_cost, length, 0 AS source, 0 AS target
  INTO geodata.networklines_1_0
  FROM django.routing_networklines_1_0;

SELECT id, ST_Force3D(ST_Transform(ST_Force2D(st_geometryN(geom, 1)),3857)) AS geom,
  network_type, cost::FLOAT, 0.0 AS reverse_cost, length, 0 AS source, 0 AS target
  INTO geodata.networklines_2_0
  FROM django.routing_networklines_2_0;

SELECT id, ST_Force3D(ST_Transform(ST_Force2D(st_geometryN(geom, 1)),3857)) AS geom,
  network_type, cost::FLOAT, 0.0 AS reverse_cost, length, 0 AS source, 0 AS target
  INTO geodata.networklines_3_0
  FROM django.routing_networklines_3_0;

-- fill the 3rd coordinate according to their floor number

UPDATE geodata.networklines_0_0 SET geom=ST_Translate(ST_Force3DZ(geom),0,0,0);
UPDATE geodata.networklines_1_0 SET geom=ST_Translate(ST_Force3DZ(geom),0,0,1);
UPDATE geodata.networklines_2_0 SET geom=ST_Translate(ST_Force3DZ(geom),0,0,2);
UPDATE geodata.networklines_3_0 SET geom=ST_Translate(ST_Force3DZ(geom),0,0,3);

UPDATE geodata.networklines_0_0 SET length =ST_Length(geom) where geom is not null;
UPDATE geodata.networklines_1_0 SET length =ST_Length(geom) where geom is not null;
UPDATE geodata.networklines_2_0 SET length =ST_Length(geom) where geom is not null;
UPDATE geodata.networklines_3_0 SET length =ST_Length(geom) where geom is not null;

-- no cost should be 0 or NULL/empty
UPDATE geodata.networklines_0_0 SET cost=1 WHERE cost=0 or cost IS NULL;
UPDATE geodata.networklines_1_0 SET cost=1 WHERE cost=0 or cost IS NULL;
UPDATE geodata.networklines_2_0 SET cost=1 WHERE cost=0 or cost IS NULL;
UPDATE geodata.networklines_3_0 SET cost=1 WHERE cost=0 or cost IS NULL;


-- update unique ids id accordingly
UPDATE geodata.networklines_0_0 SET id=id+200000;
UPDATE geodata.networklines_1_0 SET id=id+300000;
UPDATE geodata.networklines_2_0 SET id=id+400000;
UPDATE geodata.networklines_3_0 SET id=id+500000;
...
DELETE from geodata.networklines_3857 where length = 0;
