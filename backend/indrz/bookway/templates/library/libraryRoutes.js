...
function library_book_position(book_key) {

    return indrzApiCall('/indrz/api/v1/library/location/' + book_key).done(function (data) {
        // If successful

        routeLocalData.start = {};
        routeLocalData.start.xcoord = 1587954.504;
        routeLocalData.start.ycoord = 5879616.571;
        routeLocalData.start.floor = 1;
        ...
        routeToBook("Bibliothek Eingang", data.properties.name, routeStartValue, routeEndValue, 0);
        ...
        data.properties.name = data.properties.key
        open_popup(data.properties, popup_location, data.properties.name );

        libraryMarker(popup_location);
...
function routeToBook(startName, endName, startCoords, EndCoords, routeType) {
    ...
    var geoJsonUrl = baseApiUrl + "directions/" + startCoords + "&" + EndCoords + "&" + routeType + '/?format=json';
    ...
    indrzApiCall(geoJsonUrl).then(function (response) {
        ...
        document.getElementById('route-to').value = library_key;
        document.getElementById('route-from').value = startName;

        insertRouteDescriptionText(startName, library_key, routeData, false);
        ...
        globalRouteInfo.endName = library_key;
        globalRouteInfo.poiStartId = "noid";
        globalRouteInfo.poiEndId = "noid";
        globalRouteInfo.routeUrl = "/?campus=1&key=" + library_key;
