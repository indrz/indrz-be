...
var poi_cat_name = "{{ poi_cat_name }}";
var poi_cat_id = {{poi_cat_id}};
var poi_start_id = "{{ poi_start_id }}";
var poi_end_id = "{{ poi_end_id }}";
var poiActive = false;
...
var route_from_spaceid = "{{route_from_spaceid}}";
var route_to_spaceid = "{{route_to_spaceid}}";
var route_from_xyz = "{{route_from_xyz}}";
var route_to_xyz = "{{route_to_xyz}}";
var route_to_poi_id = "{{route_to_poi_id}}"
var route_from_poi_id = "{{route_to_poi_id}}"
var route_type = "{{route_type}}";
var centerx = "{{centerx}}";
var centery = "{{centery}}";
var tempStart = [];
var tempEnd = [];
...
var foo = {};
var globalBookInfo = {};
var globalPopupInfo = {"poiId":"noid", "poiCatId": "noid", "poiIdPopup": null, "poiCatName": null, "poiCatShareUrl": null,
    "bookId": false, "bookCoords": false, "name": false, "roomcode": null, 'src':null, 'floor':null, 'coords': null,
 "spaceid":null, "wmsInfo": null};
var globalRouteInfo = {"routeUrl": null,"endSpaceId": null, "startSpaceId": null, "endPoiId": null,
    "startPoiId": null, "bookUrl": null, "startCoord": null, "endCoord": null};
var globalSearchInfo = {"searchText": false, "searchResult": false}
var temp_route_data = {};
var current_poi_id = 0;


// var share_xy = "{{ share_xy }}"  // an array like [1826602.52,6142514.22]
var share_xy = [1587942.2647,5879551.6586];
var StartCenterX = 1587942.2647;
var StartCenterY = 5879551.6586;
...
function styleIndrzAutocomplete(dataSource){
    ...
    var person ="/static/homepage/img/autocomplete_person.png";
    var org = "/static/homepage/img/autocomplete_org.png";
    var room = "/static/homepage/img/autocomplete_room.png";
    var poi = "/static/homepage/img/autocomplete_poi.png";
    var icon = "/static/homepage/img/autocomplete_poi.png";
        ...
        if(dataSource.hasOwnProperty('roomcode') && dataSource.roomcode !== "" && dataSource.roomcode !== 'undefined'){
        ...
        if(dataSource !== "sorry nothing found"){
                    var res = '<p id="indrzAutoComplete"><img style="margin-right:5px; height:20px; width:20px;" src=' + icon + '>' + dataSource.name + '<span style="font-size: 10px; color:#337ab7;">' + extraText + '</span> </p>';

        }else{
                    var res = '<p id="indrzAutoComplete" >'+ gettext("Sorry nothing found") + '</p>';
...
$('#search-input').typeahead(null, {
    ...
    templates: {
        empty: gettext("Sorry nothing found"),
...
$('#search-input').on('typeahead:selected', function (e, item) {
    ...
    // searchIndrz(building_id, gettext(item.name), 18);
    searchIndrz(building_id, item.name, 20);
    globalSearchInfo.searchText = item.name;
    fixContentHeight();

    // if(req_locale == "de"){
    //     searchIndrz(building_id, item.name_nde), 18);
    // }else{
    //     searchIndrz(building_id, item.name, 18);
    // }
...
$("#enterRoute").on("click", function (event) {
  ...
  if (globalRouteInfo.startSpaceId && globalRouteInfo.endSpaceId){
      ...
      getDirections2(globalRouteInfo.startSpaceId, globalRouteInfo.endSpaceId, '0', 'spaceIdToSpaceId')
      ...
      routeToPoiFromPoi(globalRouteInfo.startPoiId, globalRouteInfo.endPoiId)
        ...
        $.when(fetcherMd(startSearchText), fetcherMd(endSearchText)).then(function (a, b) {
            ...
            if (b[0] && b[0].features[0] && b[0].features[0].properties && b[0].features[0].properties.centerGeometry) {
