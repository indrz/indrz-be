...
urlpatterns = [
    url(
        r'(?P<start_xy>startxy=[-]?\d+\.?\d+,\d+\.\d+)&(?P<floor>floor=[-]?\d+)(?P<poi_cat_id>&poiCatId=\d{1,5})(?P<reversed>&reversed=(true|false))?/$',
        route_to_nearest_poi, name='routing-from-poi'),
    url(
        r'(?P<start_poi_id>poi-id=\d{1,5})&(?P<end_xyz>xyz=[-]?\d+\.?\d+,\d+\.\d+)&(?P<z_floor>floor=[-]?\d+)&?(?P<reversed_dir>reversed=(true|false))?/$',
    ...
    url(
        r'^foo/(?P<start_poi_id>start-poi-id=\d{1,5})&(?P<end_poi_id>end-poi-id=\d{1,5})$',
    ...
    url(
        r'(?P<space_id>space-id=\d{1,5})&(?P<poi_id>poi-id=\d{1,5})(?P<route_type>&type=\d{1,5})?/$',
